...
NOTEBOOK = ROOT / "notebooks" / "celebdf_arcface_robustness_kaggle.ipynb"
BUILDER = ROOT / "scripts" / "build_celebdf_robustness_kaggle_notebook.py"
...
class RobustnessKaggleNotebookTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.builder = load_module("build_celebdf_robustness_kaggle_notebook", BUILDER)
    ...
    def test_kaggle_privacy_and_protocol_guards_are_present(self):
        source = "\n".join(
        ...
        self.assertIn("EXPECTED_EXTRACTED_VIDEO_BYTES = 946501150", source)
        self.assertIn('input_mode = "kaggle_auto_extracted_mp4"', source)
        self.assertIn("compression=zipfile.ZIP_STORED", source)
        self.assertIn('runtime_zip.with_suffix(".zip.partial")', source)
    ...
    def test_python_code_cells_compile(self):
        for index, cell in enumerate(self.notebook["cells"]):
            ...
            if source.lstrip().startswith("# 2. 라이브러리 설치"):
                continue
            try:
                compile(source, f"kaggle-notebook-cell-{index}", "exec")
